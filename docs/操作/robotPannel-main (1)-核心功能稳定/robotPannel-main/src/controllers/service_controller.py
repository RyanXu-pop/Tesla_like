        ...
        if not self.app_state.navigation_running:
            ...
            try:
                ...
                if success:
                    ...
                    self.app_state.chassis_running = False
                    self.show_info.emit("Navigation", msg)
                    self.button_enable.emit("start_navigation_button", True)
                    return
                self.status_message.emit("Status: failed to stop navigation")
                self.show_error.emit("Navigation", f"Failed to stop navigation: {msg}")
                self.button_enable.emit("start_navigation_button", True)
                return
                self.show_info.emit("导航", "导航已关闭")
                if success:
                    self.show_info.emit("å¯¼èˆª", msg)
                else:
                    self.status_message.emit("çŠ¶æ€: å…³é—­å¯¼èˆªå¤±è´¥")
                    self.show_error.emit("å¯¼èˆª", f"å…³é—­å¯¼èˆªå¤±è´¥: {msg}")
                self.button_enable.emit("start_navigation_button", True)
            except Exception as e:
