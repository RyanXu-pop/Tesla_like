...
class WorkflowController(QObject):
    """
    Coordinates longer async workflows across SSH and map sync tasks.
    """
    ...
    async def execute_mapping_workflow(self):
        chassis_ok, _ = await self.start_service_async("chassis")
        if not chassis_ok:
            return
    ...
    async def _upload_local_map(self) -> Tuple[bool, str]:
        ...
        for candidate_yaml in yaml_files:
            ...
            candidate_pgm = os.path.join(maps_dir, pgm_name)
        ...
        if not yaml_path or not pgm_path:
            tried = [os.path.basename(f) for f in yaml_files]
    ...
    async def execute_navigation_workflow(self):
        chassis_ok, chassis_msg = await self.start_service_async("chassis")
        if not chassis_ok:
            self.workflow_finished.emit("navigation", False, f"底盘启动失败: {chassis_msg}")
            return

        await asyncio.sleep(1)

        upload_ok, upload_msg = await self._upload_local_map()
        if not upload_ok:
    ...
    async def execute_mqtt_workflow(self):
        chassis_ok, chassis_msg = await self.start_service_async("chassis")
        if not chassis_ok:
            self.workflow_finished.emit("mqtt", False, f"底盘启动失败: {chassis_msg}")
            return
        await asyncio.sleep(1)
